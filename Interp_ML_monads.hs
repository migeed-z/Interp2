...
interp_m (App e0 e1) env   = m_bind (interp_m e0 env)
                                     (\v0 -> m_bind (interp_m e1 env)
                                                     (\v1 -> (fOf v0) v1))
interp_m (Get i) env       = m_bind (m_read i)
                                    (\v0 -> ret (I v0))
...
test = [ 1     == nOf (run (interp_m (Num 1) [])),
         ...
         5     == nOf (run (interp_m (App (Lam "x" (Get 0)) (Put 5)) [])),
         6     == nOf (run (interp_m (Add (Num 1) (App (Lam "x" (Get 0)) (Put 5))) [])),
         False == bOf (run (interp_m (Gt (Num 1) (App (Lam "x" (Get 0)) (Put 5))) [])),
         6     == nOf (run (interp_m (Add (App (Lam "x" (Get 0)) (Put 2))
                                          (App (Lam "x" (Get 1)) (Put 4))) [])) ]
